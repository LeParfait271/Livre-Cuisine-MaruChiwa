...
function CartPanel({ open, onClose, recipe, factor }) {
  ...
  if (!open) return null;
  return h('div', { className: 'modal-backdrop', onMouseDown: onClose },
    h('section', { className: 'modal-panel', role: 'dialog', 'aria-modal': 'true', onMouseDown: event => event.stopPropagation() },
      h('div', { className: 'modal-head' },
        h('div', null, h('p', { className: 'eyebrow' }, 'Liste de courses'), h('h2', null, recipe.title)),
        h('button', { type: 'button', className: 'icon-btn', onClick: onClose, 'aria-label': 'Fermer' }, '×')
      ),
      h('pre', { className: 'cart-output' }, text),
      h('div', { className: 'modal-actions' },
        h(Button, { variant: 'primary', onClick: () => copyText(text).then(() => setCopied(true)) }, copied ? 'Copié' : 'Copier les courses')
      )
    )
  );
}
    ...
    h('div', { className: 'recipe-detail-grid' },
      h('section', { className: 'recipe-panel ingredients-panel' },
        h('div', { className: 'panel-heading' },
          ...
          h('div', { className: 'factor-control', 'aria-label': 'Multiplier les quantités' },
      ...
      h('section', { className: 'recipe-panel steps-panel' },
        h('div', { className: 'panel-heading' },
      ...
      h('aside', { className: 'recipe-panel notes-panel' },
        ...
        (recipe.notes || []).length
          ? h('ul', null, recipe.notes.map((note, index) => h('li', { key: `${recipe.id}:note:${index}`, dangerouslySetInnerHTML: { __html: note } })))
...
function App() {
  ...
  const [favorites, setFavorites] = useState(() => readJson('mc_food_favorites', readJson('cuisine_favs', [])));
  const [recents, setRecents] = useState(() => readJson('mc_food_recents', readJson('cuisine_recents', [])));
  ...
  const featuredRecipes = useMemo(() => {
    const recentRecipes = recents.map(id => recipesById[id]).filter(Boolean);
    const seasonRecipes = recipes.filter(recipe => (recipe.seasons || []).includes(currentSeason));
    const imageRecipes = recipes.filter(recipe => recipe.image);
    return uniqRecipes([...recentRecipes, ...seasonRecipes, ...imageRecipes, ...recipes]).slice(0, 3);
  }, [currentSeason, recents, recipes, recipesById]);
  ...
  function persistFavorites(next) {
    setFavorites(next);
    writeJson('mc_food_favorites', next);
  }
  ...
  function openRecipe(id) {
    ...
    writeJson('mc_food_recents', nextRecents);
...
requestAnimationFrame(() => {
  if (window.__gremoireReady) {
    window.__gremoireReady();
    return;
  }
